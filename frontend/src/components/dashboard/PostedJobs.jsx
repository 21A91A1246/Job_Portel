...
const PostedJobs = () => {
    ...
    const fetchJobs = async () => {
      try {
        const res = await fetch(`${process.env.REACT_APP_API_BASE_URL}/api/employer/jobs`, {
          headers: {
  ...
  const fetchApplicants = async (jobId) => {
    try {
      const res = await fetch(
        `${process.env.REACT_APP_API_BASE_URL}/api/employer/applicants/${jobId}`,
        {
                            ...
                            <ul>
                              {applicants.map((app) => (
                                <li key={app._id} style={{ marginLeft: '20px' }}>
                                  ...
                                  {app.resumePath && (
                                    <a
                                      href={`${process.env.REACT_APP_API_BASE_URL}/${app.resumePath}`}
                                      target="_blank"
